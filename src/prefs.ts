...
// keep this import above the `resource:///org/gnome/Shell/...` ones: the
// legacy build's import converter mis-parses a braced relative import that
// follows a braced resource:// import
import { MoCatalog, chineseCatalogFor } from './utils/moCatalog';
import { validateLayouts } from './components/layout/layoutValidation';
...
const RESOURCES_PREFIX = "/org/gnome/Shell/Extensions/autotile"; // must match the prefix in resources.gresources.xml

let _chineseCatalog: MoCatalog | null = null;
...
function _loadChineseCatalog(extensionPath: string, domain: string): void {
    _chineseCatalog = null;
    try {
...
function _t(msgid: string): string {
    return _chineseCatalog?.gettext(msgid) ?? _(msgid);
}

export default class AutoTileExtensionPreferences extends ExtensionPreferences {
    ...
    fillPreferencesWindow(window: Adw.PreferencesWindow): Promise<void> {
        _loadChineseCatalog(this.path, this.metadata['gettext-domain']);
        ...
        const appearenceGroup = new Adw.PreferencesGroup({
            title: _t('Appearance'),
            description: _t('Configure the appearance of AutoTile'),
        });
        ...
        const outerGapsRow = this._buildSpinButtonRow(
            ...
            _t('Outer gaps'),
            _t('Gaps between a window and the monitor borders'),
        );
        appearenceGroup.add(outerGapsRow);

        const blurRow = new Adw.ExpanderRow({
            title: _t('Blur (experimental feature)'),
            subtitle: _t(
        ...
        blurRow.add_row(
            this._buildSwitchRow(
                ...
                _t('Snap Assistant'),
                _t('Apply blur effect to Snap Assistant'),
            ),
        );
        blurRow.add_row(
            this._buildSwitchRow(
                Settings.KEY_ENABLE_BLUR_SELECTED_TILEPREVIEW,
                _t('Selected tile preview'),
                _t('Apply blur effect to selected tile preview'),
            ),
        );

        const windowBorderExpanderRow = new Adw.ExpanderRow({
            title: _t('Window border'),
            subtitle: _t('Show a border around focused window'),
        });
        ...
        windowBorderExpanderRow.add_row(
            this._buildSwitchRow(
                ...
                _t('Enable'),
                _t('Show a border around focused window'),
            ),
        );
        windowBorderExpanderRow.add_row(
            this._buildSwitchRow(
                Settings.KEY_ENABLE_SMART_WINDOW_BORDER_RADIUS,
                _t('Smart border radius'),
                _t("Dynamically adapt to the window's actual border radius"),
            ),
        );
        windowBorderExpanderRow.add_row(
        ...
        const windowBorderColorRow = new Adw.ActionRow({
            title: _t('Border color'),
            subtitle: _t('Choose the color of the border'),
        });
        ...
        const animationsRow = new Adw.ExpanderRow({
            title: _t('Animations'),
            subtitle: _t('Customize animations'),
        });
        appearenceGroup.add(animationsRow);
        animationsRow.add_row(
            this._buildSpinButtonRow(
                Settings.KEY_SNAP_ASSISTANT_ANIMATION_TIME,
                _t('Snap assistant animation time'),
                _t('The snap assistant animation time in milliseconds'),
                0,
                2000,
            ),
        );
        animationsRow.add_row(
            this._buildSpinButtonRow(
                ...
                _t('Tiles animation time'),
                _t('The tiles animation time in milliseconds'),
                0,
                2000,
            ),
        );
        ...
        const enableTilingSystemRow = this._buildSwitchRow(
            ...
            _t('Enable Tiling System'),
            _t('Hold the activation key while moving a window to tile it'),
        ...
        const tilingSystemDeactivationRow = this._buildDropDownRow(
            _t('Tiling System deactivation key'),
            _t(
        ...
        const spanMultipleTilesRow = this._buildSwitchRow(
            ...
            _t('Span multiple tiles'),
            _t('Hold the activation key to span multiple tiles'),
            this._buildActivationKeysDropDown(
        ...
        const autoTilingRow = this._buildSwitchRow(
            ...
            _t('Enable Auto Tiling'),
            _t('Automatically tile new windows to the best tile'),
        );
        behaviourGroup.add(autoTilingRow);

        const autoTilingOtherWorkspacesRow = this._buildSwitchRow(
            Settings.KEY_ENABLE_AUTO_TILING_OTHER_WORKSPACES,
            _t('Search other workspaces when full'),
            _t(
                'When every monitor is full, move a new window to a free tile on another workspace and switch to it',
            ),
        );
        Settings.bind(
            Settings.KEY_ENABLE_AUTO_TILING,
            autoTilingOtherWorkspacesRow,
            'sensitive',
        );
        behaviourGroup.add(autoTilingOtherWorkspacesRow);

        const autoFillFreedTilesRow = this._buildSwitchRow(
            Settings.KEY_ENABLE_AUTO_FILL_FREED_TILES,
            _t('Fill freed tiles from the next workspaces'),
            _t(
                'When a tiled window is closed or moved away, fill the freed tiles of its workspace by moving windows from the workspaces to the right, one after another, until no more windows are available',
            ),
        );
        behaviourGroup.add(autoFillFreedTilesRow);

        const resizeComplementingRow = this._buildSwitchRow(
            Settings.KEY_RESIZE_COMPLEMENTING_WINDOWS,
            _t('Enable auto-resize of the complementing tiled windows'),
            _t(
        ...
        const overrideWindowMenuRow = this._buildSwitchRow(
            ...
            _t('Add snap assistant and auto-tile buttons to window menu'),
            _t(
        ...
        const overrideAltTabRow = this._buildSwitchRow(
            ...
            _t('Add tiled windows to ALT+TAB menu'),
            _t(
        ...
        const raiseTogetherRow = this._buildSwitchRow(
            ...
            _t('Raise tiled windows together'),
            _t(
        ...
        const topEdgeMaximize = this._buildSwitchRow(
            ...
            _t('Drag against top edge to maximize window'),
            _t('Drag windows against the top edge to maximize them'),
        );
        Settings.bind(
            Settings.KEY_ACTIVE_SCREEN_EDGES,
            topEdgeMaximize,
        ...
        const quarterTiling = this._buildScaleRow(
            _t('Quarter tiling activation area'),
            _t('Activation area to trigger quarter tiling (%% of the screen)'),
            (sc: Gtk.Scale) => {
        ...
        const edgeTilingOffset = this._buildScaleRow(
            _t('Edge tiling offset'),
            _t('Offset from the screen edge to trigger edge tiling (in pixels)'),
            (sc: Gtk.Scale) => {
        ...
        const windowsSuggestionsGroup = new Adw.PreferencesGroup({
            title: _t('Windows suggestions'),
            description: _t('Enable and disable windows suggestions'),
        });
        prefsPage.add(behaviourGroup);

        const tilingSystemWindowSuggestionRow = this._buildSwitchRow(
            Settings.KEY_ENABLE_TILING_SYSTEM_WINDOWS_SUGGESTIONS,
            _t('Enable window suggestions for the tiling system'),
            _t(
        ...
        const snapAssistWindowSuggestionRow = this._buildSwitchRow(
            ...
            _t('Enable window suggestions for the snap assistant'),
            _t(
        ...
        const screenEdgesWindowSuggestionRow = this._buildSwitchRow(
            ...
            _t('Enable window suggestions for screen edge tiling'),
            _t(
        ...
        const layoutsGroup = new Adw.PreferencesGroup({
            title: _t('Layouts'),
            description: _t('Configure the layouts of AutoTile'),
        });
        ...
        const exportLayoutsBtn = this._buildButtonRow(
            ...
            () => {
                ...
                fc.set_current_name('autotile-layouts.json');
                fc.show();
            },
        );
        ...
        const importLayoutsBtn = this._buildButtonRow(
            _t('Import layouts'),
            _t('Import layouts'),
            _t('Import layouts from a file'),
            () => {
                const fc = this._buildFileChooserDialog(
                    _t('Select layouts file'),
                    ...
                    (_source: Gtk.FileChooserNative, response_id: number) => {
                        try {
                            if (response_id === Gtk.ResponseType.ACCEPT) {
                                ...
                                if (success) {
                                    const importedLayouts = validateLayouts(
                                        JSON.parse(
        ...
        const resetBtn = this._buildButtonRow(
            _t('Reset layouts'),
            _t('Reset layouts'),
            _t('Bring back the default layouts'),
            () => {
        ...
        const keybindings: [
            ...
            [
                ...
                _t('Move window to right tile'), // title
                _t('Move the focused window to the tile on its right'), // subtitle
                false, // is set
                true, // is on main page
            ],
            [
                Settings.SETTING_MOVE_WINDOW_LEFT,
                _t('Move window to left tile'),
                _t('Move the focused window to the tile on its left'),
                false,
                true,
            ],
            [
                Settings.SETTING_MOVE_WINDOW_UP,
                _t('Move window to tile above'),
                _t('Move the focused window to the tile above'),
                false,
                true,
            ],
            [
                Settings.SETTING_MOVE_WINDOW_DOWN,
                _t('Move window to tile below'),
                _t('Move the focused window to the tile below'),
                false,
                true,
            ],
            [
                Settings.SETTING_SPAN_WINDOW_RIGHT,
                _t('Span window to right tile'),
                _t('Span the focused window to the tile on its right'),
                false,
                false,
            ],
            [
                Settings.SETTING_SPAN_WINDOW_LEFT,
                _t('Span window to left tile'),
                _t('Span the focused window to the tile on its left'),
                false,
                false,
            ],
            [
                Settings.SETTING_SPAN_WINDOW_UP,
                _t('Span window above'),
                _t('Span the focused window to the tile above'),
                false,
                false,
            ],
            [
                ...
                _t('Span window down'),
                _t('Span the focused window to the tile below'),
                false,
                false,
            ],
            [
                Settings.SETTING_SPAN_WINDOW_ALL_TILES,
                _t('Span window to all tiles'),
                _t('Span the focused window to all the tiles'),
                false,
                false,
            ],
            [
                Settings.SETTING_UNTILE_WINDOW,
                _t('Untile focused window'),
                undefined,
                false,
                false,
            ],
            [
                ...
                _t('Move window to the center'), // title
                _t('Move the focused window to the center of the screen'), // subtitle
                false, // is set
                false, // is on main page
            ],
            [
                Settings.SETTING_FOCUS_WINDOW_RIGHT,
                _t('Focus window to the right'),
                _t(
            ...
            [
                ...
                _t('Focus window to the left'),
                _t('Focus the window to the left of the current focused window'),
                false,
                false,
            ],
            [
                Settings.SETTING_FOCUS_WINDOW_UP,
                _t('Focus window above'),
                _t('Focus the window above the current focused window'),
                false,
                false,
            ],
            [
                Settings.SETTING_FOCUS_WINDOW_DOWN,
                _t('Focus window below'),
                _t('Focus the window below the current focused window'),
                false,
                false,
            ],
            [
                Settings.SETTING_FOCUS_WINDOW_NEXT,
                _t('Focus next window'),
                _t('Focus the window next to the current focused window'),
                false,
                false,
            ],
            [
                Settings.SETTING_FOCUS_WINDOW_PREV,
                _t('Focus previous window'),
                _t('Focus the window prior to the current focused window'),
                false,
                false,
            ],
            [
            ...
            [
                ...
                _t('Cycle layouts'),
                _t('Cycle through available workspace layouts'),
                false,
                false,
            ],
        ];
        ...
        for (let i = 0; i < keybindings.length; i++) {
            keybindings[i][3] =
                (gioSettings.get_strv(keybindings[i][0])[0]?.length ?? 0) > 0;
        }
        ...
        const openKeybindingsDialogRow = new Adw.ActionRow({
            title: _t('View and Customize all the Shortcuts'),
            activatable: true,
        });
        ...
        const keybindingsPage = new Adw.PreferencesPage({
            name: _t('View and Customize Shortcuts'),
            title: _t('View and Customize Shortcuts'),
            iconName: 'dialog-information-symbolic',
        });
        ...
        const wrapAroundRow = this._buildSwitchRow(
            ...
            _t('Enable next/previous window focus to wrap around'),
            _t(
        ...
        const directionalFocusTiledWindows = this._buildSwitchRow(
            ...
            _t('Restrict directional focus to tiled windows'),
            _t(
        ...
        const importExportGroup = new Adw.PreferencesGroup({
            title: _t('Import, export and reset'),
            description: _t(
                'Import, export and reset the settings of AutoTile',
            ),
        });
        ...
        const exportSettingsBtn = this._buildButtonRow(
            _t('Export settings'),
            _t('Export settings'),
            _t('Export settings to a file'),
            () => {
                const fc = this._buildFileChooserDialog(
                    _t('Export settings to a text file'),
                    Gtk.FileChooserAction.SAVE,
                    window,
                    _t('Save'),
                    _t('Cancel'),
                ...
                fc.set_current_name('autotile-settings.txt');
                fc.show();
            },
        );
        ...
        const importSettingsBtn = this._buildButtonRow(
            _t('Import settings'),
            _t('Import settings'),
            _t('Import settings from a file'),
            () => {
                const fc = this._buildFileChooserDialog(
                    _t('Select a text file to import from'),
                    Gtk.FileChooserAction.OPEN,
                    window,
                    _t('Open'),
                    _t('Cancel'),
        ...
        const resetSettingsBtn = this._buildButtonRow(
            _t('Reset settings'),
            _t('Reset settings'),
            _t('Bring back the default settings'),
        ...
        buttons.append(
            this._buildLinkButton(
                _t('Report a bug'),
                'https://github.com/kylelee/AutoTile/issues/new?template=bug_report.md',
            ),
        );
        buttons.append(
            this._buildLinkButton(
                _t('Request a feature'),
                'https://github.com/kylelee/AutoTile/issues/new?template=feature_request.md',
            ),
        );
        ...
        footerGroup.add(
            new Gtk.Label({
                label: `${_t('Open a new issue on')} <a href="https://github.com/kylelee/AutoTile">GitHub</a>!`,
                useMarkup: true,
                margin_bottom: 32,
            }),
        );
        ...
        if (this.metadata['version-name']) {
            footerGroup.add(
                new Gtk.Label({
                    label: `· AutoTile v${this.metadata['version-name']} ·`,
                }),
            );
        }
    ...
    _buildEdgeTilingBehaviourRow(currentMode: EdgeTilingMode, onModeChange: (newMode: EdgeTilingMode) => void) {
        const row = new Adw.ActionRow({
            activatable: false,
            title: _t("Choose how windows snap to screen edges"),
        ...
        const defaultBtn = this._createEdgeTilingBehaviourOption(
            _t('Default'),
            _t('Follow quarters or screen halves'),
        ...
        const adaptiveBtn = this._createEdgeTilingBehaviourOption(
            _t('Adaptive'),
            _t('Follow corners of selected layout or screen halves'),
        ...
        const granularBtn = this._createEdgeTilingBehaviourOption(
            _t('Granular'),
            _t('Follow currently selected layout'),
    ...
    _buildCustomColorDropDown(
        ...
        const options = new Gtk.StringList();
        options.append(_t('Choose custom color')); // true
        options.append(_t('Use system accent color')); // false
        const dropdown = new Gtk.DropDown({
            model: options,
...
const ShortcutSettingButton = class extends Gtk.Button {
    ...
    constructor(settingsKey: string, gioSettings: Gio.Settings, backwardKey: string | undefined = undefined) {
        ...
        this._label = new Gtk.ShortcutLabel({
            disabled_text: _t('New accelerator…'),
            valign: Gtk.Align.CENTER,
            hexpand: false,
            vexpand: false,
        });
    ...
    _onActivated(widget: Gtk.Widget) {
        ...
        const content = new Adw.StatusPage({
            title: _t('New accelerator…'),
            // description: this._description,
            icon_name: 'preferences-desktop-keyboard-shortcuts-symbolic',
            description: _t('Use Backspace to clear'),
        });
